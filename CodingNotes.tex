...
\begin{enumerate}
	\item Initialize result as null
	\item While loop as long as head is not null 
	\item Set next as head.next
	\item Set head.next as result
	\item Set result as head
	\item Set head as next
	\item Return result
\end{enumerate}

\begin{lstlisting}[title=Solution reverseList, captionpos=t]
...
\pagebreak
\medskip   
\subsection {Delete Node in a Linked List}
https://leetcode.com/problems/delete-node-in-a-linked-list/

\begin{enumerate}
	\item Set node.val to node.next.val
	\item Set node.next as node.next.next
...
\begin{enumerate}
	\item Initialize current and reverse to empty strings
	\item While loop (head) is not null 
	\item Set current += head.val
	\item Set reverse to head.val + reverse 
	\item Set head to head.next
	\item Return current === reverse
\end{enumerate}

\begin{lstlisting}[title=Solution isPalindrome, captionpos=t]
...
%START OF SECTION - SLIDING WINDOW
...
\section{Sliding Window}
 ...
 %START  
\medskip   
\subsection {Longest Nice Substring}
https://leetcode.com/problems/longest-nice-substring/

\begin{enumerate}
	\item Initialize arr as split s and create a new Set(arr)
	\item Initialize char, sub1, and sub2 as empty strings
	\item If s is shorter than 2 return empty string
	\item Loop through arr.length starting from zero 
	\item Set char to arr[i], if set has char upper and lowercase continue
	\item Recursive call to set sub1 setting s.substring 0, i
	\item Recursive call to set sub2 setting s.substring i + 1
	\item Return sub1.length >= sub2.length ? sub1 : sub2 (longer one)
	\item Return s
\end{enumerate}

\begin{lstlisting}[title=Solution longestNiceSubstring, captionpos=t]
...
\pagebreak
\medskip   
\subsection {Find the K-Beauty of a Number}
https://leetcode.com/problems/find-the-k-beauty-of-a-number/

\begin{enumerate}
	\item Initialize result to zero 
	\item Turn number into a string so I can slice it and use it for for loop
	\item Turn it back into a number so I can compare with modulo (or let js do it automatically)
	\item If nums modulo substring === 0 result ++ 
	\item Return result 
\end{enumerate}

\begin{lstlisting}[title=Solution divisorSubstrings, captionpos=t]
...
\pagebreak
\medskip   
\subsection {Maximum Average Subarray I}
https://leetcode.com/problems/maximum-average-subarray-i/

\begin{enumerate}
	\item Initialize sum as zero 
	\item Loop through nums until k adding value to sum
	\item Set max to sum
	\item Loop through nums again updating sum to be sum-nums[i-k]+nums[i]
	\item Set max to biggest value of max and sum
	\item Return max divided by k 
\end{enumerate}

\begin{lstlisting}[title=Solution findMaxAverage, captionpos=t]
...
\begin{enumerate}
	\item Create new Map()
	\item For loop from zero till nums.length 
	\item If i - map.get(nums[i]) <= k return true
	\item Else map.set(nums[i], i)
	\item Return false
\end{enumerate}

\begin{lstlisting}[title=Solution containsNearbyDuplicate, captionpos=t]
...
\pagebreak
\medskip   
\subsection {Max Consecutive Ones III}
https://leetcode.com/problems/max-consecutive-ones-iii/

\begin{enumerate}
	\item Initialize result, count, and start at 0
	\item For loop and if nums[i] === 0 increment count
	\item If count is less than k start++ and if nums[start] === 0 decrement count 
	\item Set result to math.max result, i - start + 1 and return
\end{enumerate}

\begin{lstlisting}[title=Solution longestOnes, captionpos=t]
...
\section{Graphs}
 ...
 %START  
\medskip   
\subsection {Find Center of Star Graph}
...
\begin{lstlisting}[title=Solution findCenter, captionpos=t]
...
\pagebreak
\medskip   
\subsection {Find if Path Exists in Graph}
...
\begin{lstlisting}[title=Solution validPath, captionpos=t]
...
\pagebreak
\medskip   
\subsection {Find the Town Judge}
...
\begin{lstlisting}[title=Solution findJudge, captionpos=t]
...
\pagebreak
\medskip   
\subsection {All Paths From Source to Target}
...
\begin{lstlisting}[title=Solution allPathsSourceTarget, captionpos=t]
...
\begin{lstlisting}[title=Solution canVisitAllRooms, captionpos=t]
...
\section{Trie}
 ...
 %START  
\medskip   
\subsection {Index Pairs of a String}
...
\begin{lstlisting}[title=Solution indexPairs, captionpos=t]
...
\pagebreak
\medskip   
\subsection {Search Suggestions System}
...
\begin{lstlisting}[title=Solution suggestedProducts, captionpos=t]
...
\begin{lstlisting}[title=Solution FileSystem, captionpos=t]
class FileSystem {
    constructor(){
        this.cache = {'': -1};
    }
...
\pagebreak
\medskip   
\subsection {Short Encoding of Words}
...
\begin{lstlisting}[title=Solution minimumLengthEncoding, captionpos=t]
...
\pagebreak
\medskip   
\subsection {Top K Frequent Words}
...
\begin{lstlisting}[title=Solution topKFrequent, captionpos=t]
...
\begin{lstlisting}[title=Solution reverseString, captionpos=t]
...
\begin{lstlisting}[title=Solution isPowerOfTwo, captionpos=t]
...
\begin{lstlisting}[title=Solution removeElements, captionpos=t]
...
\pagebreak
\medskip   
\subsection {All Possible Full Binary Trees}
...
\begin{lstlisting}[title=Solution allPossibleFBT, captionpos=t]
...
\pagebreak
\medskip   
\subsection {Find the Winner of the Circular Game}
...
\begin{lstlisting}[title=Solution findTheWinner, captionpos=t]
...
\begin{lstlisting}[title=Solution calPoints, captionpos=t]
const calPoints = (ops) => {
...
\pagebreak
\medskip   
\subsection {Remove All Adjacent Duplicates In String}
...
\begin{lstlisting}[title=Solution removeDuplicates, captionpos=t]
...
\pagebreak
\medskip   
\subsection {Implement Queue using Stacks}
...
\begin{lstlisting}[title=Solution isValid, captionpos=t]
const isValid = (s) => {
...
\pagebreak
\medskip   
\subsection {Buildings With an Ocean View}
...
\begin{lstlisting}[title=Solution findBuildings, captionpos=t]
...
\section{Heap}
 ...
 %START  
\medskip   
\subsection {Maximum Product of Two Elements in an Array}
...
\begin{lstlisting}[title=Solution maxProduct, captionpos=t]
...
\medskip %END
 ...
 \pagebreak
\medskip   
\subsection {The K Weakest Rows in a Matrix}
...
\begin{lstlisting}[title=Solution kWeakestRows, captionpos=t]
...
\begin{lstlisting}[title=Solution kWeakestRowsHeap, captionpos=t]
...
\begin{lstlisting}[title=Solution lastStoneWeight, captionpos=t]
...
\begin{lstlisting}[title=Solution findRelativeRanks, captionpos=t]
...
\begin{lstlisting}[title=Solution KthLargest, captionpos=t]
...
\begin{lstlisting}[title=Solution flipAndInvertImage, captionpos=t]
...
\begin{lstlisting}[title=Solution flipAndInvertImage1Line, captionpos=t]
...
\pagebreak
\medskip   
\subsection {Matrix Diagonal Sum}
...
\begin{lstlisting}[title=Solution diagonalSum, captionpos=t]
...
\pagebreak
\medskip   
\subsection {Count Negative Numbers in a Sorted Matrix}
...
\begin{lstlisting}[title=Solution countNegatives, captionpos=t]
...
\begin{lstlisting}[title=Solution countNegativesFlat, captionpos=t]
...
\begin{lstlisting}[title=Solution shiftGrid, captionpos=t]
...
\begin{lstlisting}[title=Solution transpose, captionpos=t]
...
\section{Two Pointers}
 ...
 %START  
\medskip   
\subsection {Reverse Words in a String III}
...
\begin{lstlisting}[title=Solution reverseWords, captionpos=t]
...
\begin{lstlisting}[title=Solution reverseWordsSplit, captionpos=t]
...
\pagebreak
\medskip   
\subsection {Remove Palindromic Subsequences}
...
\begin{lstlisting}[title=Solution removePalindromeSub, captionpos=t]
...
\pagebreak
\medskip   
\subsection {Number of Arithmetic Triplets}
...
\begin{lstlisting}[title=Solution arithmeticTriplets, captionpos=t]
...
\begin{lstlisting}[title=Solution intersection, captionpos=t]
...
\begin{lstlisting}[title=Solution moveZeroes, captionpos=t]
